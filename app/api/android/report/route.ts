...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('[android/report POST]', err)
...
export async function GET(req: NextRequest) {
  try {
    ...
    const devices = db.prepare('SELECT * FROM android_devices ORDER BY last_seen DESC').all()
    return NextResponse.json({ devices })
  } catch (err) {
    console.error('[android/report GET]', err)
    return NextResponse.json(
      { error: String(err), detail: err instanceof Error ? err.stack : undefined },
      { status: 500 }
    )
